...
def setup(password):
	...
	try:
		...
		p4.input = p4.run("user","-o","applications")[0]
		p4.run("user","-i","-f")
		# modify protects to make it an admin user
		table = p4.run("protect", "-o")[0]
		table["Protections"].append("admin user applications * //...")
		p4.input = table
		p4.run("protect", "-i")
		# set up a long timeout group for the applications user
